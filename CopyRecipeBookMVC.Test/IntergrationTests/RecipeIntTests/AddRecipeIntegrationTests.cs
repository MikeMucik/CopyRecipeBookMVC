...
namespace CopyRecipeBookMVC.Application.Test.IntergrationTests.RecipeIntTests
{
	...
	{		
		...
		[Fact]
		public void AddInvalidDataTheSameId_AddRecipe_ShouldNotAddToColletion()
		{
			//Arrange
			var recipe = new NewRecipeVm
			{
		...
		[Fact]
		public void AddMissingData_AddRecipe_ShouldNotAddToColletion()
		{
			//Arrange
			var recipe = new NewRecipeVm
			{
			...
			var exception = Assert.Throws<Microsoft.EntityFrameworkCore.DbUpdateException>(() => _recipeService.AddRecipe(recipe));
			//Assert			
			//Assert.Equal("Nieprawidłowe dane", exception.Message);
		}
		[Fact]
